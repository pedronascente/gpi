...
class MigrarCaptacao extends MX_Controller {
    ...
    public function migrarCaptacaoSave(){
         ...
         $vendedor_receptor= $this->MCaptacao->select_vendedor($this->input->post('migrar'),'vendedor_receptor');
         $vendedor_doador= $this->MCaptacao->select_vendedor($this->input->post('migrar'),'vendedor_doador');
         $data_inicial = $this->input->post('migrar')['data_inicial'];
         $data_fim = $this->input->post('migrar')['data_fim'];
         $mensagem = "<p>O Vendedor : <b>{$vendedor_receptor[0]->nome} </b> Recebeu =  <b>{$total_captacao}</b>  captações.  Do Vendedor: <b>{$vendedor_doador[0]->nome} </b>  No periodo  : <b>{$data_inicial} até {$data_fim}</b> .</p>";
