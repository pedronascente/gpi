<?php
    $array_plano = array(
        ...
        'Rastreamento + Assistência Veicular',
            ...
            <div class="row">
                <div class="col-xs-6 col-sm-3 col-md-3 ">
                    <div class="form-group">
                        <label>Placa:</label>
                        <input  type="text" name="placa" id="placa"  style="text-transform:uppercase" maxlength="10" value="<?= (!empty($veiculo['placa'])) ? strtoupper($veiculo['placa']) : ""; ?>" class=" form-control"  <?= $statusCadastro == 2 && !empty($veiculo) == 2 ? Funcoes::verificarCamposContratoVeiculo($camposVeiculos, "placa", $id_veiculo) : ""; ?>/>
                    </div>
                </div>  
                    ...
                    <div class="form-group">
                        ...
                        <select name="tipo_seguro" class="form-control"  id="_tipo_seguro"  >
                            <?php for ( $i=0;$i<count($array_plano);$i++){ ?>
                                    <option 
                                        value="<?php echo $array_plano[$i] ?>" 
                                        <?=(isset($veiculo['tipo_seguro']) && ($array_plano[$i] == $veiculo['tipo_seguro'])) ? "selected" : '';?> >
                                        <?=$array_plano[$i];?>
                                    </option>
                            <?php } ?>
                        </select>
                        ...
                        <div class="input-group" >
                            ...
                            if ($tipo_cadastro == "contrato_promocao") :
                                ...
                                <input type="text" 
                                    ...
                                    id="taxa_instalacao"  
                                    value="<?= (!empty($veiculo['taxa_instalacao'])) ? $veiculo['taxa_instalacao'] : ""; ?>" 
                                    class=" mask_real  form-control" <?= $statusCadastro == 2 && !empty($veiculo) == 2 ? Funcoes::verificarCamposContratoVeiculo($camposVeiculos, "taxa_instalacao", $id_veiculo) : ""; ?>  required="" />
                            <?php
                            endif;
                            ?>
                            <span class="input-group-addon">.00</span>
                ...
                <div class="col-xs-4 col-sm-4 col-md-4 ">
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            <span class="input-group-addon">$</span>
                            <input type="text"    
                                name="taxa_monitoramento" 
                                id="taxa_monitoramento" 
                                size="20" 
                                value="<?=(!empty($veiculo['taxa_monitoramento'])) ? $veiculo['taxa_monitoramento'] : ""; ?>" 
                                class=" mask_real form-control"  <?= $statusCadastro == 2 && !empty($veiculo) == 2 ? Funcoes::verificarCamposContratoVeiculo($camposVeiculos, "taxa_monitoramento", $id_veiculo) : ""; ?>  required="" />
                            <span class="input-group-addon">.00</span>
                        </div>
                    </div>
                </div>  
...
<script type="text/javascript">
    /**
     * 
     * @Dscription :  Responsavel por habilitar os campos imputs :
     * @label : Taxa Mensal Proteção Veicular - name = valor_protecao
     * @label : Taxa Mensal Assistencial Veicular   - name = valor_protecao_assistencial
     * @evento :  change() : select[name = "tipo_seguro"]
     * 
     **/
    $(function()
    {
        ...
        else if(_glolbal_tipo_seguro == "Rastreamento + Assistência Veicular")
        ...
        $('#_tipo_seguro').change(function()
        {
            ...
            else if(_valor == "Rastreamento + Assistência Veicular")
            {
               $('#_valor_protecao1').hide();
               $('#_valor_protecao2').show();
