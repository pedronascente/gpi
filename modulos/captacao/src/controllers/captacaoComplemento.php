...
function enviarSMS($dados_sms)
{
    ...
    Funcoes::enviaSMS(array(
        ...
        'mensagem' => 'Obrigado pela preferencia. Em breve um de nossos consultores entrara em contato.  Volpato', 'telefone' => $sms_captacao_telefone1
    ), new Sms, new HumanMultipleSend(
        "volpatoapipos",
        "bmHvfLRFlr"
    ));
}
...
if (!empty($id_vendedor_com_captcao_registrada)) {
    ...
    header('location:../../../../index.php?pg=1&r=1');
    exit;
} else {
    $Dados['captacao_telefone1'] = str_replace(" ", "", str_replace("-", "", $Dados['captacao_telefone1']));
    $dados_sms['captacao_telefone1'] = $Dados['captacao_telefone1'];
