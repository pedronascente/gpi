...
final class Captacao extends Crud {
	...
	private $captacaoCampos = array("captacao_cliente", "origem","captacao_email", "captacao_telefone1", "captacao_telefone2", "captacao_telefone3", "captacao_interesse");
	...
	public function insertCaptacaoDeletada($dados) {
		...
		unset($dados['captacao_assunto']);
		unset($dados['ddd1']);
		unset($dados['captacao_responsavel']);
		unset($dados['captacao_ja_e_cliente']);
		unset($dados['captacao_tipo_servico']);
		unset($dados['captacao_localizacao_do_servico_atual']);
		unset($dados['captacao_cliente_desde']);
		unset($dados['captacao_pendencias_financeiras']);
		unset($dados['captacao_acoes']);
		unset($dados['captacao_conceito']);
	...
	public function selCaptacaoEDEDED($id_captacao) {
		$this->_sql = "
		SELECT
		captacao_id
		captacao_id_usuario,
	...
	public function buscarRegraUsuario($id_usuario) {
		...
		LEFT JOIN captacao_niveis cn 
		ON cnu.`captacao_niveis_usuarios_captacao_niveis_id` = cn.`captacao_niveis_id`
		WHERE cnu.`captacao_niveis_usuarios_id_usuario` = {$id_usuario}  AND u.status_captacao ='off'";
		$this->Read()->FullRead($this->_sql);
		return $this->Read()->getResult();
	}
    ...
    public function selectMenorIdUsuarioTableUsuario($nivelPermissao, $status,$limit='' ,$vendedor='') {
    	if($limit){
    		$limit = ' LIMIT 0,1';
    	}

    	if($vendedor){
    		$AND = " AND u.id !={$vendedor}";
    	}else{
    		$AND=NULL;
    	}
    	...
    	WHERE 
    	u.status_captacao = '" . $status . "'  
    	AND (pu.id_permissaouser = 8 ) 
    	AND u.ativo = 1 
    	AND CN.captacao_niveis_ra = {$nivelPermissao}
    	{$AND}    
    ...
    public function selectViaturasAntigas($dados, $limite = NULL) {
    	...
    	$limite = !empty($limite) ? "LIMIT " . $limite : "";
    	$this->_sql = "SELECT * FROM viaturas WHERE MONTH(data) = '{$month}'  AND  YEAR(DATA) = '{$ano}'     ORDER BY 'data' DESC {$limite}";
    	$this->Read()->FullRead($this->_sql);
    	return $this->Read()->getResult();
    }
...
/*
----------------------------------------------------------------------
CORRIGIR BUD CAPTACAO DUPLICADA :
----------------------------------------------------------------------
*/

/*
1) CONTAR TOTAL  DE  CAPTAÇÕES REGISTRADAS QUE TEM O E-MAIL =  ? e interesse =  ?  
*/
public function count_captacao_duplicada($captacao_email,$captacao_interesse){
	$this->_sql = "SELECT COUNT(captacao_id) as total_captacao_duplicada FROM captacao WHERE captacao_email ='{$captacao_email}'  AND  captacao_interesse = {$captacao_interesse} ";
	$this->Read()->FullRead($this->_sql);
	return $this->limparArray($this->Read()->getResult());
}


public function get_captacao_por_telefone($captacao_telefone1){
	$this->_sql = "SELECT captacao_id_usuario FROM captacao WHERE captacao_telefone1 = '{$captacao_telefone1}'   ORDER BY captacao_id ASC LIMIT 1";
	$this->Read()->FullRead($this->_sql);
	$_resultato =  $this->Read()->getResult();
...
/*
2) SELECIONAR A PRIMEIRA CAPTAÇÕES REGISTRADAS QUE TEM O E-MAIL = ? e interesse =  ? 
*/
public function get_primeiro_registro($captacao_email,$captacao_interesse){
	$this->_sql = "SELECT captacao_id FROM captacao WHERE captacao_email = '{$captacao_email}'  AND  captacao_interesse = {$captacao_interesse}  ORDER BY captacao_id ASC LIMIT 1";
	$this->Read()->FullRead($this->_sql);
	return $this->Read()->getResult()[0];
}

/*
3) SELECIONAR TODOS OS USUARIOS QUE RECEBERAM CAPTAÇÃO REGISTRADA COM  E-MAIL = ? e interesse = ? . MENOS O PRIMEIRO USUARIO .
*/
public function  get_usuarios_duplicados($captacao_email,$captacao_interesse,$primeira_captacao){
	$this->_sql = "SELECT captacao_id_usuario  FROM  captacao WHERE captacao_email = '{$captacao_email}'  AND captacao_interesse = {$captacao_interesse} AND captacao_id <> {$primeira_captacao} ORDER BY captacao_id ASC";
	$this->Read()->FullRead($this->_sql);
	return $this->Read()->getResult();
}

/*
4) DELETAR TODAS AS CAPTAÇÕES DUPLICADAS QUE FORAM REGISTRADAS APOS O PRIMEIRO REGISTRO ? 
*/
public function  delete_captacao_duplicadas($captacao_email,$captacao_interesse,$primeira_captacao){
	$this->Delete()->ExDelete($this->_tabela, "WHERE captacao_email = '{$captacao_email}' AND  captacao_interesse = {$captacao_interesse} AND captacao_id <> {$primeira_captacao} ", null);
	return $this->Delete()->getResult();
}
