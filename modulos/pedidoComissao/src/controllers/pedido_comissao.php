...
switch ($acao) {
    case "UPLOADXML":
        ...
        $dados_arquivo = $_FILES['arquivo'];
        $dados['pedido_comissao_comissao1'] = Funcoes::moeda(str_replace('R$ ', '', '0,80'));
        ...
        if (!empty($_FILES['arquivo']['tmp_name'])) {
            ...
            foreach ($Linhas as  $linha) {
                ...
                $dados['pedido_comissao_n_os'] = $linha->getElementsByTagName("Data")->item(0)->nodeValue;
                $dd = explode('/', $linha->getElementsByTagName("Data")->item(1)->nodeValue);
                $dados['pedido_comissao_data'] = $dd[2] . '-' . $dd[1] . '-' . $dd[0];
                $dados['pedido_comissao_cliente'] = $linha->getElementsByTagName("Data")->item(2)->nodeValue;
                $dados['pedido_comissao_conta'] = $linha->getElementsByTagName("Data")->item(3)->nodeValue;
                $dados['pedido_comissao_servico'] = $linha->getElementsByTagName("Data")->item(4)->nodeValue;

                if ($primeir_linha == false) {
                    $pc->insert($dados);
                }
                $primeir_linha = false;
            }

            $pedido_comissao_id_usuario =  $dados['pedido_comissao_id_usuario'];
            //  var_dump($pedido_comissao_id_usuario);

            header("Location: ../../../../index.php?pg=6&id_u=" . $dados['pedido_comissao_id_usuario'] . "&acao=visualizar&page=" . $page . "&id_setor=" . $setor);
        } else {

            die('Atenção : verifique seu arquivo! somente (.xml)');
        }

        break;

    case "AddPedidoComissao":
        $page = !empty($dados['page']) ? $dados['page'] : 6;
        $setor = $dados['id_setor'];
        unset($dados['page'], $dados['id_setor']);
        ...
        $dados['pedido_comissao_servico'] = !empty($dados['pedido_comissao_servico']) ?    $dados['pedido_comissao_servico'] : '';
        $dados['pedido_comissao_captacao'] = !empty($dados['pedido_comissao_captacao']) ?    $dados['pedido_comissao_captacao'] : '';
    ...
    case "editarComissao":
        ...
        $dados['pedido_comissao_data'] = Funcoes::FormatadataSql($dados['pedido_comissao_data']);
        $dados['pedido_comissao_conta'] = $dados['pedido_comissao_conta'];
        $dados['pedido_comissao_placa'] = !empty($dados['pedido_comissao_placa']) ? $dados['pedido_comissao_placa'] : null;
        $dados['pedido_comissao_comissao1'] = !empty($dados['pedido_comissao_comissao1']) ? Funcoes::moeda(str_replace('R$ ', '', $dados['pedido_comissao_comissao1'])) : null;
        $dados['pedido_comissao_comissao2'] = !empty($dados['pedido_comissao_comissao2']) ? Funcoes::moeda(str_replace('R$ ', '', $dados['pedido_comissao_comissao2'])) : null;
        $dados['pedido_comissao_comissao3'] = !empty($dados['pedido_comissao_comissao3']) ? Funcoes::moeda(str_replace('R$ ', '', $dados['pedido_comissao_comissao3'])) : null;
    ...
    */
            ...
            foreach ($lista as $li) :
                ...
                $html .= '
                                                                                                              <tr align="left">
                                                                                                                              ...
                                                                                                                              <td>' . ($li["pcf_empresa"]) . '</td>
                                                                                                                              <td>' . ($li["setor_local"]) . '</td>
    ...
    case 'insertPlanilha':
        ...
        $anterior = null;
        $explode_data = explode('_', $dados['pcf_nome']);
        $dados['pcf_nome'] = $explode_data[0];
        #empresa_usuario;
        switch ($explode_data[1]) {
            case '1':
                $dados['pcf_empresa'] = 'VPSP';
                break;
            case '2':
                $dados['pcf_empresa'] = 'VH';
                break;
            case '3':
                $dados['pcf_empresa'] = 'VP - Alarmes';
                break;
            case '4':
                $dados['pcf_empresa'] = 'VP - Guaíba';
                break;
            case '5':
                $dados['pcf_empresa'] = 'Volpmann Matriz';
                break;
            case '6':
                $dados['pcf_empresa'] = 'Volpmann - Filial';
                break;
            case '7':
                $dados['pcf_empresa'] = 'Volpato';
                break;
            case '8':
                $dados['pcf_empresa'] = 'Volpato - Tramandaí';
                break;
            case '9':
                $dados['pcf_empresa'] = 'Volpato - Filial';
                break;
            case '10':
                $dados['pcf_empresa'] = 'Easyseg';
                break;
        }
    ...
    case "gerarComissoes":
        ...
        if ($inicial == 12 && $final == 1) {
            ...
            $final = $ano_seguinte . "-" . $final . "-16 23:59:59";
        } else {
            $inicial = $ano . "-" . $inicial . "-16 00:00:00";
        ...
        if (!empty($contratos)) {
            foreach ($contratos as $con) {
                ...
                foreach ($veiculos as $v) {
                    ...
                    $valoresSeguros['Rastreamento'] = 35.00;
                    ...
                    $valoresSeguros['Rastreamento + Proteção veicular + Assistência Veicular'] = 65.00;
                    ...
                    $DadosComissoes['pedido_comissao_comissao1'] = $valoresSeguros[$v['tipo_seguro']];
                    ...
                    $DadosComissoes['pedido_comissao_servico'] = '';
                    $DadosComissoes['pedido_comissao_cliente'] = '';
                    $DadosComissoes['pedido_comissao_captacao'] = '';
    ...
    case "desarquivarPlanilha":
        $retorno = (new PedidoComissaoFuncionario())->desarquivarPlanilha(array('pcf_id' => $dados['pcf_id'], 'pcf_arquivar' => 0,));
